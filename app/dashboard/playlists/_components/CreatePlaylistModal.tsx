...
import { createPlaylist, getPlaylistCoverPresignedUrl } from "@/app/actions/playlist";
...
export default function CreatePlaylistModal({ onClose }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    if (coverBlob) {
      ...
      if ("error" in res) { setError(res.error); setSaving(false); setUploading(false); return; }
      ...
      setUploading(false);
      if (!up.ok) { setError("Ошибка загрузки обложки. Попробуйте снова."); setSaving(false); return; }
      coverKey = res.fileKey;
    }

    const result = await createPlaylist({ name, description, isPublic, coverKey });
    setSaving(false);
        ...
        >
          ...
          <div className="flex justify-center mb-6">
            <button
              ...
              className="relative w-40 h-40 rounded-2xl overflow-hidden bg-white/[0.03] border border-white/[0.07] flex flex-col items-center justify-center gap-2 group hover:border-white/20 transition-colors"
            >
              {coverPreview ? (
